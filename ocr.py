...
class PororoOcr:
    ...
    def run_ocr(self, img_path, debug: bool = False):
...
if __name__ == "__main__":
    ...
    # Install the opener to be used for all HTTPS requests
    urllib.request.install_opener(opener)
    print("ocr 들어가기 전")
    ocr = PororoOcr()
    print("ocr 들어간 후")

    image_list = convert_from_path('test/개인정보.pdf')

    print(type(image_list[0]))
    print("run_ocr중")
    print(image_list[0].size)
    text = ocr.run_ocr(image_list[1], debug=True)
    # text = ocr.run_ocr("test/kor_image_1.png", debug=True)
